...
1. 稠密多项式与稀疏多项式的区别
稠密多项式的主要区别在于`term_list`的表示方式不同：
    * 稠密多项式的`term_list`是所有系数的直接列表。
    * 稀疏多项式的`term_list`是链表形式，每个节点包含阶数和系数组成的项。

2. 实现方法
    * 通过选择函数屏蔽`term_list`的差异，重新实现与`term_list`相关的函数（如`order_dense`、`coeff_dense`等）。
    * 共享其他通用函数（如`make_poly`、`variable`等）。


#### 代码输出
```rust
p1 = (dense:1x^4 + 2x^3 + 0x^2 + 3x^1 + -2)
p2 = (dense:5x^3 + 0x^2 + -1x^1 + 4)
p1 + p2 = (dense:1x^4 + 7x^3 + 0x^2 + 2x^1 + 2)
p1 - p2 = (dense:1x^4 + -3x^3 + 0x^2 + 4x^1 + -6)
p1 * p2 = (dense:5x^7 + 10x^6 + -1x^5 + 17x^4 + -2x^3 + -3x^2 + 14x^1 + -8)
-p1 = (dense:-1x^4 + -2x^3 + 0x^2 + -3x^1 + 2)
```
#### 代码实现
```rust
